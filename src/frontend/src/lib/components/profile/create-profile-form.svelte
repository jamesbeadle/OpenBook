<script lang="ts">
  ...
  /*
    $: isSubmitDisabled = $profile === null || !isUsernameValid($profile?.username) || 
        ...
        || !isNameValid($profile?.lastName) || !isUsernameValid($profile?.openChatUsername)
        || !isEmailValid($profile?.emailAddress) || !isPhoneValid($profile?.phoneNumber);
*/
...
{#if $profile}
  <div class="flex my-2 flex-col">
    ...
    <p class="text-sm text-red-600">The information your provide in this form will be public.</p>
  </div>
